...
`noted` is a Swift menubar app and ships an on-device Swift ASR stack inherited from HushScribe. V1 supports three backends behind an `ASRBackend` protocol:

- `whisperkit` — Whisper Large v3 via [WhisperKit](https://github.com/argmaxinc/WhisperKit). **Default**, chosen for multilingual robustness.
- `fluidaudio-parakeet` — Parakeet-TDT v3 via [FluidAudio](https://github.com/FluidInference/FluidAudio). Selectable fast-path for English-only sessions.
- `sfspeech` — Apple Speech framework. Fallback.

The manifest's `transcription.asr_backend` field records which backend was used for the session. Model size / variant (e.g., Whisper Base vs Large, Parakeet checkpoint) is a `noted` setting, not a per-session manifest value.

Python ASR (`faster-whisper`, `whisperx`) is **not** part of `noted`. The rationale for choosing the Swift stack is recorded in `ASR Stack Recommendation — Swift vs Python.md` at the project root and reflected in §27.9.
...
- Fast-stop behaviour: `stop` returns after audio is flushed; ASR and diarization run asynchronously as post-processing (Swift stack per §15.2 / §15.4, chosen model variant).
...
**Context:** In-person diarization quality is the primary technical risk. Library choice affects quality, install complexity, licence, **and the runtime boundary of `noted`** — a Python diarizer would force a second runtime (interpreter + Torch + models) inside the Swift capture agent, breaking single-binary distribution and the fast-stop guarantee.

**Options:**

- **(a) FluidAudio offline diarizer** — Swift, in-process, already integrated in `noted` via `OfflineDiarizerManager`. Good quality; slightly below best-in-class on overlapped speech.
- **(b) `pyannote.audio` via Python sidecar** — best-in-class published DER, particularly for overlapped / >3-speaker meetings, at the cost of a second runtime, heavier install, Hugging Face token management, and two sets of code-signing problems.
- **(c) `whisperx`** — superseded; wraps `faster-whisper`, requires a Python runtime that `noted` does not host.

**Decision: (a) FluidAudio offline diarizer.** Rationale recorded in `ASR Stack Recommendation — Swift vs Python.md` at the project root. Keeps `noted` a single signed Swift binary, preserves fast-stop semantics (§15, guardrail 3), avoids a Python runtime in the capture agent, and reuses the already-working HushScribe pipeline. Primary operating language is `en-AU`, where the quality delta against pyannote is smallest.

**Re-evaluation trigger:** if real-world diarization on the first real dataset falls below a useful threshold for our meeting mix, revisit. Switching later is viable because the `ASRBackend` protocol in `noted` and the opaque-label contract on the `briefing` side isolate the diarizer choice from the rest of the system.

**Blocks:** Phase 2. **Status:** Resolved.
