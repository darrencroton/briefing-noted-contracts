...
> **Status update, 2026-04-25:** The original bring-up path through Step 7 is complete. `noted` Phase 2 (N-01 through N-13), `briefing` Phase 4a ingest (B-12 through B-20), Step 7/B-21, `briefing` Phase 4b planning/watch (B-01 through B-11), `noted` Phase 3 plus N-25 (N-14 through N-20, N-25), and polish tickets N-21 through N-24 plus B-22 through B-25 are complete. Phase 5 hardening should wait for operational data from those completed flows.
...
2. **There was one silent architectural decision that needed making before anyone wrote code.** HushScribe is Swift end-to-end, but the master plan originally named Python ASR libraries (`faster-whisper`, `whisperx`). **Resolved 2026-04-23**: `noted` keeps the Swift stack (WhisperKit + FluidAudio). Rationale in `ASR Stack Recommendation — Swift vs Python.md`; master plan §15.2, §15.4, and §27.9 updated. Step 2 below is retained for audit trail.
...
|2|~~Resolve Swift-vs-Python ASR decision~~ ✅ Resolved 2026-04-23 — Swift stack (WhisperKit + FluidAudio) |You decide, dev team advises|Steps 3, 4      |
...
The authoritative triage table lives at `noted/hushscribe-triage.md` — read from the actual source, cross-checked with `rg`, and reviewed against the master plan and guardrails. That file is the single source of truth for keep/strip/rewrite decisions; do not duplicate it elsewhere.

### What “done” looks like

A triage table committed to the `noted` repo as `hushscribe-triage.md` (before any stripping). Every source file appears in the table. The team can articulate why each STRIP file is safe to remove without breaking a KEEP file.
...
## Step 2 — Resolve Swift-vs-Python ASR decision ✅ RESOLVED 2026-04-23

> **Outcome:** `noted` keeps the Swift-native stack — WhisperKit (default), FluidAudio Parakeet-TDT v3 (selectable), and the FluidAudio offline diarizer. The master plan has been updated: §15.2, §15.4, §27.9, §27.0 dependency map, §20.4 settings table, §23 Phase 2 notes, and §29 references. The manifest's `transcription.asr_backend` default is now `"whisperkit"`. Full rationale: `ASR Stack Recommendation — Swift vs Python.md` (project root).
>
> The rest of this step is kept for audit trail and because future open questions may follow the same pattern.
...
## Appendix: HushScribe module triage

**Source of truth:** [`noted/hushscribe-triage.md`](../noted/hushscribe-triage.md).

A draft table previously lived here, inferred from the architecture doc. It has been superseded by the authoritative, source-read triage in `noted/hushscribe-triage.md`, which buckets every file under `HushScribe/Sources/HushScribe/` as `KEEP`, `STRIP`, or `REWRITE` with rationale and strip-safety notes.

Do not reintroduce a table in this document. If triage calls need to change, change them in `noted/hushscribe-triage.md` and reference that file from here.

### Dependency fallout (not in the triage file)

The triage is per-file; the SPM dependency consequences are derived:

- **Remove with the strip:** `mlx-swift-lm` and its MLX model cache logic (follows from stripping `Services/LLMSummaryEngine.swift` and `Models/SummaryModel.swift`).
- **Keep:** `FluidAudio` — offline diarizer (§15.4), Silero VAD, and Parakeet-TDT v3 ASR backend (§15.2).
- **Keep:** `WhisperKit` — default ASR backend (§15.2), locked by the Swift-stack decision in Step 2.
